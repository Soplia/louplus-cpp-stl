...
namespace jrxstl {
	//class of queue
	template<class T, class Container = jrxstl::deque<T>>
	class queue {
	public:
	...
	void swap(queue<T, Container>& x, queue<T, Container>& y) {
		jrxstl::swap(x.container_, y.container_);
	}

	//class of priority_queue
	template <class T, class Container = jrxstl::vector<T>,
		class Compare = jrxstl::less<typename Container::value_type>>
		...
		public:
			...
			priority_queue(InputIterator first, InputIterator last,
				...
				jrxstl::make_heap(container_.begin(), container_.end());
			}
			bool empty() const {
			...
			void push(const value_type& val) {
				...
				jrxstl::push_heap(container_.begin(), container_.end(), compare_);
			}
			void pop() {
				jrxstl::pop_heap(container_.begin(), container_.end(), compare_);
				container_.pop_back();
			}
			void swap(priority_queue& x) {
				jrxstl::swap(container_, x.container_);
				jrxstl::swap(compare_, x.compare_);
			}
		public:
