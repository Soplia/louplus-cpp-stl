...
namespace jrxstl{
	...
	void vector<T, Alloc>::resize(size_type n, value_type val) {
		if (n < size()) {
			...
			finish_ = jrxstl::uninitialized_fill_n(finish_, lengthOfInsert, val);
		}
		else if (n > capacity()) {
			auto lengthOfInsert = n - size();
			T *newStart = dataAllocator::allocate(getNewCapacity(lengthOfInsert));
			T *newFinish = jrxstl::uninitialized_copy(begin(), end(), newStart);
			newFinish = jrxstl::uninitialized_fill_n(newFinish, lengthOfInsert, val);
	...
	void vector<T, Alloc>::reallocateAndCopy(iterator position, InputIterator first, InputIterator last) {
		...
		T *newFinish = jrxstl::uninitialized_copy(begin(), position, newStart);
		newFinish = jrxstl::uninitialized_copy(first, last, newFinish);
		newFinish = jrxstl::uninitialized_copy(position, end(), newFinish);
	...
	void vector<T, Alloc>::reallocateAndFillN(iterator position, const size_type& n, const value_type& val) {
		...
		T *newEndOfStorage = newStart + newCapacity;
		T *newFinish = jrxstl::uninitialized_copy(begin(), position, newStart);
		newFinish = jrxstl::uninitialized_fill_n(newFinish, n, val);
		newFinish = jrxstl::uninitialized_copy(position, end(), newFinish);
	...
	void vector<T, Alloc>::insert_aux(iterator position,
		...
		if (locationLeft >= locationNeed) {
			if (finish_ - position > locationNeed) {
				jrxstl::uninitialized_copy(finish_ - locationNeed, finish_, finish_);
				copy_backward(position, finish_ - locationNeed, finish_);
				copy(first, last, position);
			}
			else {
				iterator temp = jrxstl::uninitialized_copy(first + (finish_ - position), last, finish_);
				jrxstl::uninitialized_copy(position, finish_, temp);
	...
	void vector<T, Alloc>::insert_aux(iterator position, Integer n, const value_type& value, std::true_type) {
		...
		if (locationLeft >= locationNeed) {
			...
			jrxstl::uninitialized_fill_n(position, n, value);
			finish_ += locationNeed;
		}
		else {
	...
	void vector<T, Alloc>::shrink_to_fit() {
		...
		finish_ = jrxstl::uninitialized_copy(start_, finish_, t);
	...
	void vector<T, Alloc>::swap(vector& v) {
		if (this != &v) {
			jrxstl::swap(start_, v.start_);
			jrxstl::swap(finish_, v.finish_);
			jrxstl::swap(endOfStorage_, v.endOfStorage_);
		}
	}
	template<class T, class Alloc>
	...
	void vector<T, Alloc>::allocateAndFillN(const size_type n, const value_type& value) {
		...
		jrxstl::uninitialized_fill_n(start_, n, value);
	...
	void vector<T, Alloc>::allocateAndCopy(InputIterator first, InputIterator last) {
		...
		finish_ = jrxstl::uninitialized_copy(first, last, start_);
		endOfStorage_ = finish_;
	}
	template<class T, class Alloc>
	template<class InputIterator>
	...
	typename vector<T, Alloc>::size_type vector<T, Alloc>::getNewCapacity(size_type len)const {
		...
		auto res = jrxstl::max(oldCapacity, len);
