...
namespace jrxstl{
	...
	public:
		...
		public:
			...
			{//jrxstl::set的pos以1开始计数,因此加1,以下都是如此
	...
	private:   //内部使用的一些函数
		...
		{ // jrxstl里的read以1开始计数，在这里转换下,以下同理可得
		...
		try :head(0)
		{
			...
			for (size_type i = 0, j = pos + n; i < size() && j >= pos + 1; )
			{
				...
				if (str[j - 1] != '0' && str[j - 1] != '1')
					throw jrxstl::invalid_argument_1();  //非法参数，抛出异常
#endif
				set(head, i++, str[--j] == '1');
			}
		}
