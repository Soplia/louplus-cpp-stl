...
namespace jrxstl {
	namespace Detail {
		...
		void dq_iter<T>::swap(dq_iter& it) {
			jrxstl::swap(mapIndex_, it.mapIndex_);
			jrxstl::swap(cur_, it.cur_);
			//jrxstl::swap(container_, it.container_);
		}
		template<class T>
	...
	void deque<T, Alloc>::push_back(const value_type& val) {
		...
		//bug fix
		//2015.01.02
		jrxstl::construct(end_.cur_, val);
		++end_;
	}
	template<class T, class Alloc>
	void deque<T, Alloc>::push_front(const value_type& val) {
		...
		//bug fix
		//2015.01.02
		jrxstl::construct(beg_.cur_, val);
	}
	template<class T, class Alloc>
	...
	void deque<T, Alloc>::swap(deque<T, Alloc>& x) {
		jrxstl::swap(mapSize_, x.mapSize_);
		jrxstl::swap(map_, x.map_);
		beg_.swap(x.beg_);
		end_.swap(x.end_);
	}
